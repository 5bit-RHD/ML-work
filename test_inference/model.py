...
class AudioTranscriber:
    ...
    def __init__(
        self,
        model_path: str = "wav2vec2_russian_train_2.onnx",
        processor_name: str = "bond005/wav2vec2-base-ru"
    ) -> None:
        """Инициализирует AudioTranscriber.

        Args:
            model_path: Путь к ONNX модели.
            processor_name: Имя или путь к предобученному процессору Wav2Vec2.
        """
        ...
        self.ort_session = ort.InferenceSession(model_path, sess_options)
        ...
        self.numbers_dict = {
            ...
            'одиннадцать': {"count":11, "text":"одиннадцать вагонов"}, 'двенадцать': {"count":12, "text":"двенадцать вагонов"},
            'тринадцать': {"count":13, "text":"тринадцать вагонов"}, 'четырнадцать': {"count":14, "text":"четырнадцать вагонов"},
            'пятнадцать': {"count":15, "text":"пятнадцать вагонов"}, 'шестнадцать': {"count":16, "text":"шестнадцать вагонов"},
            'семнадцать': {"count":17, "text":"семнадцать вагонов"}, 'восемнадцать': {"count":18, "text":"восемнадцать вагонов"},
            'девятнадцать': {"count":19, "text":"девятнадцать вагонов"}, 'двадцать': {"count":20, "text":"двадцать"},
    ...
    def find_closest_command(self, transcription: str, threshold: int = 80) -> tuple:
        """
        Находит ближайшую команду к заданной транскрипции.

        Эта функция сначала пытается найти точное соответствие команде в словаре команд.
        Если точное соответствие не найдено, она ищет команды с числовыми атрибутами
        (например, "протянуть на 5 вагонов").

        Args:
            transcription (str): Транскрипция аудио, для которой нужно найти команду.
            threshold (int): Пороговое значение для нечеткого сравнения строк. По умолчанию 80.

        Returns:
            tuple: Кортеж, содержащий:
                - str: Найденная команда или пустая строка, если команда не найдена.
                - int: Метка команды или -1, если команда не найдена.
                - int: Числовой атрибут команды (если есть) или -1.
        """
        ...
        for word in words:
            ...
            if closest_word[1] >= 95:
                ...
                if number_words:
                    return f"{closest_word[0]} на {' '.join(number_words)}", self.count_commands_dict[command], total_count
                else:
    ...
    def predict(self, audio_path: str) -> Dict[str, float]:
        ...
        input_values = self.processor(audio_data, sampling_rate=sampling_rate, return_tensors="np").input_values
...
# result = transcriber.predict("./test.wav")
# print(result)